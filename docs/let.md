...
ES6新增了let指令，用来声明变量。它的用法类似于var，但是所声明的变量，只在let指令所在的代码块内有效。
...
只要块级作用域内存在let指令，它所声明的变量就“绑定”（binding）这个区域，不再受外部的影响。
...
ES6明确规定，如果区块中存在let和const指令，这个区块对这些指令声明的变量，从一开始就形成了封闭作用域。凡是在声明之前就使用这些指令，就会报错。

总之，在代码块内，使用let指令声明变量之前，该变量都是不可用的。这在语法上，称为“暂时性死区”（temporal dead zone，简称TDZ）。
...
上面代码中，在let指令声明变量tmp之前，都属于变量tmp的“死区”。
...
const的作用域与let指令相同：只在声明所在的块级作用域内有效。
...
由于const指令只是指向变量所在的地址，所以将一个对象声明为常量必须非常小心。
...
ES6规定，var指令和function指令声明的全局变量，属于全局对象的属性；let指令、const指令、class指令声明的全局变量，不属于全局对象的属性。
...
上面代码中，全局变量a由var指令声明，所以它是全局对象的属性；全局变量b由let指令声明，所以它不是全局对象的属性，返回undefined。
