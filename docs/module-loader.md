...
上面代码表明，ES6 模块不会缓存运行结果，而是动态地去被加载的模块取值，并且变量总是绑定其所在的模块。（`export default`是一个特例，`export default counter;`只会输出`counter`当时的值，如果需要绑定，要写作`export { counter as default };`。）
