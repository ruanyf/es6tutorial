...
（1）没有自己的`this`。箭头函数不会创建自己的`this`，它只会从自己的作用域链的上一层继承`this`。
...
* 如果是普通函数，那么`this`会指向全局对象`window`，这时应该输出`21`（非严格模式下）。
  * 如果是严格模式，则`this`会指向`undefined`，导致`this.id`代码报错。
* 但是，因为这里使用的是箭头函数，因此它会直接从作用域链的上一层（也就是`foo`）继承`this`，也就是本例的`{id: 42}`，所以打印出来的是`42`。

箭头函数的这种从父级作用域继承`this`的特性很有利于封装回调函数。下面是一个例子，DOM 事件的回调函数封装在一个对象里面。
...
上面代码的`init`方法中，使用了箭头函数。因为`init`方法的`this`是指向的`handler`，因此这个箭头函数里面的`this`，也指向`handler`对象。否则，回调函数运行时，`this.doSomething`这一行会报错，因为此时`this`指向`document`对象。

另外因为箭头函数根本没有自己的`this`，所以它不能用作构造函数。

箭头函数转成 ES5 的代码如下。可以清晰地看到箭头函数只是引用外层的`this`，并没有自己的`this`。
